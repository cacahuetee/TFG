...
def generar_carton_bingo():
    columnas = {
        ...
        'I': random.sample(range(20, 40), 5),
        'N': random.sample(range(40, 60), 5),
        'G': random.sample(range(60, 80), 5),
        'O': random.sample(range(80, 100), 5),
    }
    ...
    # Elegir 12 posiciones únicas aleatorias para dejar en blanco (puede incluir el centro)
    posiciones_vacias = set()
    while len(posiciones_vacias) < 12:
        fila = random.randint(0, 4)
        columna = random.randint(0, 4)
        posiciones_vacias.add((fila, columna))

    # Aplicar los espacios vacíos
    for fila, columna in posiciones_vacias:
        carton[fila][columna] = None
